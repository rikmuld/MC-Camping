...
public class GuiContainerCampfireCook extends GuiContainer {
	...
	protected void drawGuiContainerBackgroundLayer(float mouseX, int mouseY, int partTicks)
	{
		...
		if(fire.equipment!=null)fire.equipment.drawGuiTexture(this);
		this.drawTexturedModalRect(guiLeft+66, guiTop+94-scale, 176, 40-scale, 44, scale);
		this.drawTexturedModalRect(guiLeft+79, guiTop+83, 79, 105, 18, 18);
		...
		if(fire.equipment!=null)
		{
			for(int i = 0; i<fire.equipment.maxFood; i++)
			{
				...
				boolean isNotCooked = fire.getStackInSlot(i+2)!=null?fire.equipment.canCook(fire.getStackInSlot(i+2).itemID):false;
