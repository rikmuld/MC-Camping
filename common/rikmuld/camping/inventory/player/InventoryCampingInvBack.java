...
public class InventoryCampingInvBack extends InventoryPlayerMain{
	...
	public InventoryCampingInvBack(EntityPlayer player, int size, ArrayList<SlotDisable> slots, InventoryItemMain backpack)
	{
		...
		if(player.getEntityData().getCompoundTag("campInv").hasKey("backpack")==false) player.getEntityData().getCompoundTag("campInv").setCompoundTag("backpack", new NBTTagCompound());
		this.tag = player.getEntityData().getCompoundTag("campInv").getCompoundTag("backpack");
	}
	...
	public void onInventoryChanged()
	{
		...
		if(this.getStackInSlot(0)!=null)
		{
			...
			{
				slot.enable();
				if(!this.getStackInSlot(0).hasTagCompound())this.getStackInSlot(0).setTagCompound(new NBTTagCompound());
				backpack.tag = this.getStackInSlot(0).getTagCompound();
				backpack.readFromNBT(backpack.tag);
			}
			backpack.item = this.getStackInSlot(0);
		}
		else
		{
			for(SlotDisable slot:slots)
