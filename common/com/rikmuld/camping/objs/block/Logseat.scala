...
class Logseat(modId:String, info:ObjInfo) extends RMBlockContainer(modId, info) with WithModel with WithProperties {
  ...
  override def addCollisionBoxesToList(world: World, pos:BlockPos, state:IBlockState, axisAligned: AxisAlignedBB, list: java.util.List[AxisAlignedBB], entity: Entity) {
