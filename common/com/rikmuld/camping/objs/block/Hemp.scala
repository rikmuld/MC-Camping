...
class Hemp(modId:String, info:ObjInfo) extends RMBlock(modId, info) with IPlantable with WithModel with WithInstable with WithProperties {
  ...
  override def canStay(bd:BlockData): Boolean = ((bd.world, bd.pos.down).block==this && bd.down.state.getValue(AGE) == MetaLookup.Hemp.GROWN_BIG_BOTTOM)||bd.down.block.canSustainPlant(bd.world, bd.pos.down, EnumFacing.UP, this)
  ...
  override def updateTick(world: World, pos:BlockPos, state:IBlockState, random: Random) {
    ...
    if ((world.getLightBrightness(pos.up) * 15) >= 9) {
      ...
      if (bd.meta < MetaLookup.Hemp.GROWN_SMALL) {
        if (random.nextInt((25.0F / speed).toInt + 1) == 0) bd.setState(getStateFromMeta(bd.meta+1), 2)
      } else if (bd.meta == MetaLookup.Hemp.GROWN_SMALL) {
  ...
  override def breakBlock(world:World, pos:BlockPos, state:IBlockState) = {
    if((world, pos.down).meta==MetaLookup.Hemp.GROWN_BIG_BOTTOM)(world, pos.down).setState(getStateFromMeta(MetaLookup.Hemp.GROWN_SMALL))
  ...
  override def getDrops(world: IBlockAccess, pos:BlockPos, state:IBlockState, fortune: Int): java.util.List[ItemStack] = if (getMetaFromState(state) >= MetaLookup.Hemp.GROWN_SMALL) super.getDrops(world, pos, state, fortune) else List()
  def grow(bd:BlockData): Boolean = {
    if (bd.meta < MetaLookup.Hemp.GROWN_SMALL) bd.setState(getStateFromMeta(Math.min(MetaLookup.Hemp.GROWN_SMALL, bd.meta + new Random().nextInt(3 - bd.meta) + 1)), 2)
    else if (bd.meta == MetaLookup.Hemp.GROWN_SMALL) {
      bd.setState(getStateFromMeta(MetaLookup.Hemp.GROWN_BIG_BOTTOM), 2)
      (bd.world, bd.pos.up).setState(getStateFromMeta(MetaLookup.Hemp.GROWN_BIG_TOP), 2)
      true
    } else false
  }
}