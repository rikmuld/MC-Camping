...
public class ItemBlockLantern extends ItemBlockMain{
    ...
    {
    	if(stack.hasTagCompound())list.add("Burning time left: "+stack.getTagCompound().getInteger("time")/2+" seconds");
    }
	...
	public boolean placeBlockAt(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ, int metadata)
    {
       ...
       {
    	   if(stack.hasTagCompound()&&stack.getItemDamage()==LANTERN_ON)((TileEntityLantern)world.getBlockTileEntity(x, y, z)).burnTime = stack.getTagCompound().getInteger("time");
    	   else ((TileEntityLantern)world.getBlockTileEntity(x, y, z)).burnTime = 0;
    	   return true;
       }
       return false;
    }
