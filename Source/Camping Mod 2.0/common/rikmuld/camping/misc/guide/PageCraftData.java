...
public class PageCraftData {
	...
	private void setData()
	{
		...
		shapeless = Boolean.parseBoolean(type.getTextContent());
		...
		for(int i = 0; i < list.getLength(); i++)
		{
			if(list.item(i).getTextContent().length() > 0)
			{
				...
				{
					if(ItemInfo.id(list.item(i).getTextContent().split("/")[0]) != -1)
					{
						id = ItemInfo.id(list.item(i).getTextContent().split("/")[0]) + 256;
					}
					else if(BlockInfo.id(list.item(i).getTextContent().split("/")[0]) != -1)
					{
						id = BlockInfo.id(list.item(i).getTextContent().split("/")[0]);
					}
				}
