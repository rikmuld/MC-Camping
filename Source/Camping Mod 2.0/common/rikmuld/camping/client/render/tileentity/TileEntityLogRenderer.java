...
public class TileEntityLogRenderer extends TileEntitySpecialRenderer {
	...
	public void renderTileEntityAt(TileEntity tileentity, double x, double y, double z, float fl)
	{
		...
		if(meta == 0)
		{
			...
			if((blockInfo[ForgeDirection.NORTH.ordinal()][0] == ModBlocks.log.blockID) && (((((TileEntityLog)world.getBlockTileEntity(xPos, yPos, zPos - 1)).rotation + 1) % 2) == 0))
			{
				model.renderPart("ExtentionBack");
			}
			if((blockInfo[ForgeDirection.SOUTH.ordinal()][0] == ModBlocks.log.blockID) && (((((TileEntityLog)world.getBlockTileEntity(xPos, yPos, zPos + 1)).rotation + 1) % 2) == 0))
			{
		...
		if(meta == 1)
		{
			...
			if((blockInfo[ForgeDirection.WEST.ordinal()][0] == ModBlocks.log.blockID) && (((((TileEntityLog)world.getBlockTileEntity(xPos - 1, yPos, zPos)).rotation + 1) % 2) == 1))
			{
				model.renderPart("ExtentionRight");
			}
			if((blockInfo[ForgeDirection.EAST.ordinal()][0] == ModBlocks.log.blockID) && (((((TileEntityLog)world.getBlockTileEntity(xPos + 1, yPos, zPos)).rotation + 1) % 2) == 1))
			{
