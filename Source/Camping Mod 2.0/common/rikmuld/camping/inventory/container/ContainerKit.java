...
public class ContainerKit extends ContainerMain {
	...
	public ContainerKit(InventoryPlayer playerInv, ItemStack stack)
	{
		...
		{
			if(row == playerInv.currentItem)
			{
				addSlotToContainer(new SlotNoPickup(playerInv, row, 8 + (row * 18), 157));
			}
			else
			{
				addSlotToContainer(new Slot(playerInv, row, 8 + (row * 18), 157));
			}
		}
		inv.openChest();
	}
	...
	public void onContainerClosed(EntityPlayer player)
	{
		if(!player.worldObj.isRemote)
		{
			...
			if(CookingEquipmentList.getCookingForRecipe(items) != null)
			{
				ItemStackUtil.setCurrentPlayerItem(player, CookingEquipmentList.getCookingForRecipe(items).itemInfo);
			}
			else if(items.size() > 0)
			{
				ItemStackUtil.setCurrentPlayerItem(player, new ItemStack(ModItems.kit, 1, ItemKit.KIT_USELESS));
			}
			else
			{
				ItemStackUtil.setCurrentPlayerItem(player, new ItemStack(ModItems.kit, 1, ItemKit.KIT_EMPTY));
			}
