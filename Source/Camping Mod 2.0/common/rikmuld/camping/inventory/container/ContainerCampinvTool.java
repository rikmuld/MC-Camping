...
public class ContainerCampinvTool extends ContainerMain {
	...
	public void detectAndSendChanges()
	{
		...
		for(int i = 0; i < crafters.size(); ++i)
		{
			...
			if(lanternDamage != CampingInvUtil.getLanternDamage(player))
			{
		...
		lanternDamage = CampingInvUtil.getLanternDamage(player);
	}
	...
	{
		result.setInventorySlotContents(0, CraftingManager.getInstance().findMatchingRecipe(craftMatrix, worldObj));
	}
	...
	public ItemStack transferStackInSlot(EntityPlayer player, int i)
	{
		...
		if((slot != null) && slot.getHasStack())
		{
			...
			{
				if(!mergeItemStack(itemstack1, inv.getSizeInventory() + craftMatrix.getSizeInventory() + result.getSizeInventory(), inventorySlots.size(), true)) return null;
			}
			else
			{
				if((itemstack1.itemID == ModItems.knife.itemID) || (itemstack1.itemID == ModBlocks.lantern.blockID) || (itemstack1.itemID == Item.map.itemID))
				{
					if(!mergeItemStack(itemstack1, 10, 13, false)) return null;
				}
				else return null;
			}
