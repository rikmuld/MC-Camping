...
public class TickHandler implements ITickHandler {
	...
	public void handlePlayerWalkSpeed(EntityPlayer player, World world)
	{
		if(!world.isRemote)
		{
			if((player.getCurrentItemOrArmor(4) != null) && (player.getCurrentItemOrArmor(4).itemID == ModItems.armorFurHelmet.itemID))
			{
				playerWalkSpeedAmplifier += 0.00625F;
			}
			if((player.getCurrentItemOrArmor(3) != null) && (player.getCurrentItemOrArmor(3).itemID == ModItems.armorFurChest.itemID))
			{
				playerWalkSpeedAmplifier += 0.00625F;
			}
			if((player.getCurrentItemOrArmor(2) != null) && (player.getCurrentItemOrArmor(2).itemID == ModItems.armorFurLeg.itemID))
			{
				playerWalkSpeedAmplifier += 0.00625F;
			}
			if((player.getCurrentItemOrArmor(1) != null) && (player.getCurrentItemOrArmor(1).itemID == ModItems.armorFurBoots.itemID))
			{
				playerWalkSpeedAmplifier += 0.00625F;
			}

			if(playerWalkSpeedAmplifier >= (0.00625F * 4F))
			{
				ModAchievements.armor.addStatToPlayer(player);
			}

			if(ConfigInfoBoolean.value(ConfigInfo.ENBLED_SPEEDUP))
			{
				ReflectionHelper.setPrivateValue(PlayerCapabilities.class, player.capabilities, (playerWalkSpeed + playerWalkSpeedAmplifier), 6);
			}
	...
	public void tickEnd(EnumSet<TickType> type, Object... tickData)
	{
		...
		{
			handlePlayerWalkSpeed((EntityPlayer)tickData[0], ((EntityPlayer)tickData[0]).worldObj);
		}
	}
		...
		if(type.equals(EnumSet.of(TickType.PLAYER)))
		{
			...
			if(!world.isRemote)
			{
				for(ItemStack stack: player.inventory.mainInventory)
				{
					if(stack != null)
					{
						if(stack.isItemEqual(new ItemStack(ModItems.hemp)))
						{
							ModAchievements.hemp.addStatToPlayer(player);
						}
					}
				}
			}
			...
			if(!world.isRemote && CampingInvUtil.hasLantarn(player))
			{
				...
				if(tickLight >= 10)
				{
					...
					if(player.worldObj.getBlockId((int)player.posX, (int)player.posY - 1, (int)player.posZ) == 0)
					{
						player.worldObj.setBlock((int)player.posX, (int)player.posY - 1, (int)player.posZ, ModBlocks.light.blockID);
					}
					else if(player.worldObj.getBlockId((int)player.posX, (int)player.posY, (int)player.posZ) == 0)
					{
						player.worldObj.setBlock((int)player.posX, (int)player.posY, (int)player.posZ, ModBlocks.light.blockID);
					}
					else if(player.worldObj.getBlockId((int)player.posX, (int)player.posY + 1, (int)player.posZ) == 0)
					{
						player.worldObj.setBlock((int)player.posX, (int)player.posY + 1, (int)player.posZ, ModBlocks.light.blockID);
					}
				}
