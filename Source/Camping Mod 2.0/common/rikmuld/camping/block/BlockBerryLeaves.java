...
public class BlockBerryLeaves extends BlockMain implements IShearable {
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float par7, float par8, float par9)
	{
		if(!world.isRemote)
		{
			...
			if(tile.berries == true)
			{
				...
				{
					ModAchievements.berry2.addStatToPlayer(player);
				}
				else
				{
					ModAchievements.berry1.addStatToPlayer(player);
				}
	...
	public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entity, ItemStack stack)
	{
		...
		{
			((TileEntityBerry)world.getBlockTileEntity(x, y, z)).noDecay = true;
		}
	}
	...
	public void triggerDecay(World world, int x, int y, int z, int id)
	{
		if(id == Block.wood.blockID)
		{
			...
			for(int i = 0; i < 6; i++)
			{
				if(blocks[i][0] == Block.wood.blockID)
				{
					hasWoodOnSide = true;
				}
			}
			...
			if(!hasWoodOnSide)
			{
				((TileEntityBerry)world.getBlockTileEntity(x, y, z)).spreadDecay();
			}
		}

		boolean hasGoodLeafOnSide = false;

		if((world.getBlockTileEntity(x + 1, y, z) instanceof TileEntityBerry) && !((TileEntityBerry)world.getBlockTileEntity(x + 1, y, z)).getIsPlacedByHand())
		{
			hasGoodLeafOnSide = true;
		}
		if((world.getBlockTileEntity(x - 1, y, z) instanceof TileEntityBerry) && !((TileEntityBerry)world.getBlockTileEntity(x - 1, y, z)).getIsPlacedByHand())
		{
			hasGoodLeafOnSide = true;
		}
		if((world.getBlockTileEntity(x, y, z + 1) instanceof TileEntityBerry) && !((TileEntityBerry)world.getBlockTileEntity(x, y, z + 1)).getIsPlacedByHand())
		{
			hasGoodLeafOnSide = true;
		}
		if((world.getBlockTileEntity(x, y, z - 1) instanceof TileEntityBerry) && !((TileEntityBerry)world.getBlockTileEntity(x, y, z - 1)).getIsPlacedByHand())
		...
		if((world.getBlockTileEntity(x, y + 1, z) instanceof TileEntityBerry) && !((TileEntityBerry)world.getBlockTileEntity(x, y + 1, z)).getIsPlacedByHand())
		{
			hasGoodLeafOnSide = true;
		}
		if((world.getBlockTileEntity(x, y - 1, z) instanceof TileEntityBerry) && !((TileEntityBerry)world.getBlockTileEntity(x, y - 1, z)).getIsPlacedByHand())
		...
		if(!hasGoodLeafOnSide)
		{
			((TileEntityBerry)world.getBlockTileEntity(x, y, z)).startDecay();
		}
	}
}
