...
public class BlockLog extends BlockRotationMain {
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float par7, float par8, float par9)
	{
		...
		{
			((TileEntityLog)world.getBlockTileEntity(x, y, z)).mountable.interactFirst(player);
		}
		return true;
	}
	...
	public void setBlockBoundsBasedOnState(IBlockAccess world, int x, int y, int z)
	{
		...
		if(meta == 0)
		{
			...
			if((blockInfo[ForgeDirection.NORTH.ordinal()][0] == ModBlocks.log.blockID) && (((((TileEntityLog)world.getBlockTileEntity(x, y, z - 1)).rotation + 1) % 2) == 0))
			{
				zMin = 0;
			}
			if((blockInfo[ForgeDirection.SOUTH.ordinal()][0] == ModBlocks.log.blockID) && (((((TileEntityLog)world.getBlockTileEntity(x, y, z + 1)).rotation + 1) % 2) == 0))
			{
				zMax = 1;
			}
		}
		...
		if(meta == 1)
		{
			...
			if((blockInfo[ForgeDirection.WEST.ordinal()][0] == ModBlocks.log.blockID) && (((((TileEntityLog)world.getBlockTileEntity(x - 1, y, z)).rotation + 1) % 2) == 1))
			{
				xMin = 0;
			}
			if((blockInfo[ForgeDirection.EAST.ordinal()][0] == ModBlocks.log.blockID) && (((((TileEntityLog)world.getBlockTileEntity(x + 1, y, z)).rotation + 1) % 2) == 1))
			{
				xMax = 1;
			}
		}
