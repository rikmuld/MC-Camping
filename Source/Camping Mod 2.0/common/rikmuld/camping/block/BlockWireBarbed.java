...
public class BlockWireBarbed extends BlockMain {
	...
	public void checkSide(World world, int x, int y, int z)
	{
		...
		tile.sides[0] = world.isBlockOpaqueCube(x, y, z - 1) || (blocks[2][0] == blockID);
		tile.sides[1] = world.isBlockOpaqueCube(x, y, z + 1) || (blocks[3][0] == blockID);
		tile.sides[2] = world.isBlockOpaqueCube(x - 1, y, z) || (blocks[4][0] == blockID);
		tile.sides[3] = world.isBlockOpaqueCube(x + 1, y, z) || (blocks[5][0] == blockID);
		...
		if(entity instanceof EntityLivingBase)
		{
			if(!world.checkNoEntityCollision(AxisAlignedBB.getBoundingBox(x + minX, y + minY, z + minZ, x + maxX, y + maxY, z + maxZ)))
			{
				if(entity instanceof EntityPlayer)
				{
					...
					if((player.getCurrentItemOrArmor(1) != null) && (player.getCurrentItemOrArmor(1).getItem() instanceof ItemArmorFur))
					{
						flag++;
					}
					if((player.getCurrentItemOrArmor(2) != null) && (player.getCurrentItemOrArmor(2).getItem() instanceof ItemArmorFur))
					{
						flag++;
					}
					if((player.getCurrentItemOrArmor(3) != null) && (player.getCurrentItemOrArmor(3).getItem() instanceof ItemArmorFur))
					{
						flag++;
					}
					if((player.getCurrentItemOrArmor(4) != null) && (player.getCurrentItemOrArmor(4).getItem() instanceof ItemArmorFur))
					{
						flag++;
					}
