...
class TileEntityTent extends TileEntityInventory with TileEntityTicker {
  ...
  override def update(): Unit = {
    if(firstLoad) {
      getBlock.asInstanceOf[BlockTent].createStructure(world, pos)
      firstLoad = false
    }
  ...
  def toStack(i: Int, n: Option[Int] = None): ItemStack = (i, n.getOrElse(count(i))) match {
    case (_, 0) =>
      ItemStack.EMPTY
    case (LANTERN, count) =>
      TileEntityLantern.stackFromTime(light, count)
    case (_, count) =>
  ...
  def contentAdded(id: Int, stack: ItemStack): Unit = id match {
    case LANTERN =>
      val light = TileEntityLantern.timeFromStack(stack)
  ...
  def contentRemoved(id: Int): Unit = id match {
    ...
    case LANTERN =>
      val stack = TileEntityLantern.stackFromTime(light, 1)
