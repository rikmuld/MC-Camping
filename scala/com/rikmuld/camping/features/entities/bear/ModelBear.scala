...
class ModelBear extends ModularModal {
  setTextureSize(128, 64)

  addBox(ModelBear.LEG_1, 0, 27, 6, 11, 6, 3F, 13F, -8F, 0F, 0F, 0F)
  addBox(ModelBear.LEG_2, 0, 44, 6, 14, 6, 4F, 10F, 9F, 0F, 0F, 0F)
  addBox(ModelBear.LEG_3, 0, 27, 6, 11, 6, -9F, 13F, -8F, 0F, 0F, 0F)
  addBox(ModelBear.LEG_4, 0, 44, 6, 14, 6, -10F, 10F, 9F, 0F, 0F, 0F)
  ...
  override def setRotationAngles(f: Float, f1: Float, f2: Float, f3: Float, f4: Float, f5: Float, entity: Entity) {
    ...
    getBox(ModelBear.LEG_1).rotateAngleX = MathHelper.cos(f * 0.6662F) * 1.4F * f1
    getBox(ModelBear.LEG_2).rotateAngleX = MathHelper.cos((f * 0.6662F) + Math.PI.toFloat) * 1.4F * f1
    getBox(ModelBear.LEG_3).rotateAngleX = MathHelper.cos((f * 0.6662F) + Math.PI.toFloat) * 1.4F * f1
    getBox(ModelBear.LEG_4).rotateAngleX = MathHelper.cos(f * 0.6662F) * 1.4F * f1
  }
}