...
abstract class CookingEquipment(var cookTime: Int, var maxFood: Int, var itemInfo: ItemStack, model: ModularModal) {
...
class Grill(item: ItemStack) extends CookingEquipment(CampingMod.config.cookTimeGrill, 4, item, CookingEquipmentModels.GRILL) {
...
class Pan(item: ItemStack) extends CookingEquipment(CampingMod.config.cookTimePan, 8, item, CookingEquipmentModels.PAN) {
  override def doRenderFood(foodIndex: Int, stack: ItemStack, entity: EntityLivingBase) {}
  override def drawGuiTexture(container: GuiCampfireCook) {
...
class Spit(item: ItemStack) extends CookingEquipment(CampingMod.config.cookTimeSpit, 2, item, CookingEquipmentModels.SPIT) {
  override def doRenderFood(foodIndex: Int, stack: ItemStack, entity: EntityLivingBase) {
    val item = new ItemStack(stack.getItem, 1, stack.getItemDamage)
    GL11.glPushMatrix()
