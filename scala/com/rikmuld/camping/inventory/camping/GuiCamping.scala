...
class GuiCamping(var player: EntityPlayer) extends GuiTabbed(player, new ContainerCamping(player)) {
  ...
  override def getTabs: Seq[Tab] = Seq(
    ...
    new Tab("Backpack", 20, this, TAB_BACKPACK, new ItemStack(Objs.backpack)),
    new Tab("Crafting", 20, this, TAB_CRAFTING, new ItemStack(Objs.knife)),
  ...
  override def drawGUI(mouseX: Int, mouseY: Int) {
    ...
    if (isPointInRegion(8, 35, 16, 16, mouseX, mouseY))
      itemRender.renderItemIntoGUI(new ItemStack(Objs.backpack), guiLeft + 8, guiTop + 35)

    if (isPointInRegion(8, 53, 16, 16, mouseX, mouseY))
      itemRender.renderItemIntoGUI(new ItemStack(Objs.knife), guiLeft + 8, guiTop + 53)

    if (isPointInRegion(196, 35, 16, 16, mouseX, mouseY))
      itemRender.renderItemIntoGUI(new ItemStack(Objs.lantern), guiLeft + 196, guiTop + 35)
