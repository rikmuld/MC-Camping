...
class GuiMapHUD(mapData: MapData) extends Gui {
  GuiMapHUD.initTexture(mapData.colors)
  ...
  def drawMap(mc: Minecraft, width: Int, height: Int):Unit = {
    ...
    mc.renderEngine.bindTexture(GuiMapHUD.TEX_UTILS)

    drawTexturedModalRect((width * 2) - 133, 5, 0, 42, 128, 128)
    GL11.glScalef(2F, 2F, 2F)

    GL11.glBindTexture(GL11.GL_TEXTURE_2D, GuiMapHUD.TEX_MAP)
  ...
  def drawPlayer(mc: Minecraft, width: Int, height: Int, player: EntityPlayer): Unit = {
    ...
    if (mc.player == player) mc.renderEngine.bindTexture(GuiMapHUD.TEX_RED_DOT)
    else mc.renderEngine.bindTexture(GuiMapHUD.TEX_BLUE_DOT)
